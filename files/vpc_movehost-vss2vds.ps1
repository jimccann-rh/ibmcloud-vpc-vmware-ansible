...
Function Invoke-MigratevSS2vDS {
    ...
    Param (
        ...
        Get-VDPortgroup -VDSwitch $MgmtvDS -Name $MgmtvDSPG
        ...
        $config.Portgroup[0].Spec.Name = $vswitch_network_label 
        ...
        $config.ProxySwitch[0].Spec.Backing.PnicSpec[0].UplinkPortKey = $hostport.key
#        $config.ProxySwitch[0].Spec.Backing.PnicSpec[0].UplinkPortKey = $hostport.key[0]
        $config.ProxySwitch[0].Spec.Backing.PnicSpec[0].UplinkPortgroupKey = $hostport.PortgroupKey
        write-output ( $hostport | Format-List | Out-String)
        $changeMode = 'modify'
